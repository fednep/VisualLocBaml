...
namespace TranslationApi.Baml
{
    ...
    internal class ResourceTextReader : IDisposable 
    {
        ...
        private TextReader __Reader;     // internal text reader
        private int __Delimiter;  // delimiter
        ...
        {
            if (__Columns!= null && index < __Columns.Count && index >= 0)
            {
                return (string) __Columns[index];
            }
            else 
            {
                return null;
        ...
        {
            int _char;
            while ((_char = __Reader.Read())>=0)
            {
                if (_char == '\n')
                {
                    continue; // continue if it is '\n'
                }
                else if (_char == '\r' && __Reader.Peek() == '\n')
                {
