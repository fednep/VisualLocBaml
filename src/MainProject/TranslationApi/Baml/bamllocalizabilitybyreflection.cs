...
namespace TranslationApi.Baml
{
    ...
    public class BamlLocalizabilityByReflection : BamlLocalizabilityResolver
    {
        ...
        // private members
        //---------------------------     
        private Dictionary<string, Assembly> __Assemblies;   // the _assemblies table
        private Dictionary<string, Type> __TypeCache;    // the types cache

        // well know assembly names, keep them sorted.
        private static readonly string[] __WellKnownAssemblyNames = new string[] { 
                "presentationcore", 
        ...
        private static Assembly[] __WellKnownAssemblies = new Assembly[__WellKnownAssemblyNames.Length];
        ...
        public BamlLocalizabilityByReflection(Assembly[] assemblies)
        {
            if (assemblies != null)
            {
                // create the table
                __Assemblies = new Dictionary<string, Assembly>(assemblies.Length);
                ...
                {                    
                    ...
                    for (int i = 0; i < assemblies.Length; i++)
                    {
                        ...
                        if (assemblies[i] != null)
                        {
                            ...
                            __Assemblies[assemblies[i].GetName().FullName] = assemblies[i];
                        }
                    }
                }
                finally
            ...
            __TypeCache = new Dictionary<string, Type>(32);
        }        
        ...
        private Type GetType(string assemblyName, string className)
        {
            ...
            if (__TypeCache.ContainsKey(fullName))
            {
                // we found it in the cache, so just return
                return __TypeCache[fullName];
            }            

            // we didn't find it in the table. So let's get to the assembly first
            Assembly assembly = null;
            if (__Assemblies != null && __Assemblies.ContainsKey(assemblyName))
            {
                // find the assembly in the hash table first
                assembly = __Assemblies[assemblyName];
            }
            ...
            if (assembly == null)
            {
                   ...
                   )
                {
                    ...
                    if (__WellKnownAssemblies[index] == null)
                    {
                        // it is a well known name, load it from the gac
                        __WellKnownAssemblies[index] = Assembly.Load(assemblyName);
                    }         
                    
                    assembly = __WellKnownAssemblies[index];                                                            
                }                
            }
        ...
        private static string GetCompatibleAssemblyName(string shortName)
        {
            ...
            {                
                if (__WellKnownAssemblies[i] != null)
                {
                    ...
                    asmName = __WellKnownAssemblies[i].GetName();
